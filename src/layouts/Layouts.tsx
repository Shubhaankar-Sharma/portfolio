...
export const PageLayout = ({ children }: LayoutProps) => {
  ...
  const getLinkNames = () => {
    ...
    const rightLink =
      currentIndex < paths.length - 1 ? paths[currentIndex + 1] : "";
    return {
      left: leftLink,
      right: rightLink,
      leftName:
        leftLink === "/"
          ? "Home"
          : leftLink.charAt(1).toUpperCase() + leftLink.slice(2),
      rightName: rightLink
        ? rightLink.charAt(1).toUpperCase() + rightLink.slice(2)
        : "",
    };
  };

  const { left, right, leftName, rightName } = getLinkNames();
